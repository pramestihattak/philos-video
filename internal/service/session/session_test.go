...
// newTestSessionService creates a Service with a known secret and expiry.
func newTestSessionService(t *testing.T) *session.Service {
	t.Helper()
	svc, err := session.New(nil, nil, "test-secret-32-chars-long!!!!!", "1h")
	if err != nil {
...
func TestNewSessionService_InvalidExpiry(t *testing.T) {
	_, err := session.New(nil, nil, "test-secret-32-chars-long!!!!!", "not-a-duration")
	if err == nil {
...
func TestParseToken_ValidToken(t *testing.T) {
	...
	svc, err := session.New(nil, nil, secret, "1h")
	if err != nil {
		t.Fatalf("session.New: %v", err)
	}

	claims := session.PlaybackClaims{
...
func TestParseToken_WrongSecret(t *testing.T) {
	svc, err := session.New(nil, nil, "correct-secret-32-chars-long!!!!", "1h")
	if err != nil {
		t.Fatalf("session.New: %v", err)
	}
